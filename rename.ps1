...
function Rename {
	...
	Ls $TargetFolder -Recurse | Where { $_.GetType().Name -eq $dirType -and ($_.Name.Contains($PlaceHolderCompanyName) -or $_.Name.Contains($PlaceHolderProjectName)) } | ForEach-Object{
		...
		$newDirectoryName=$_.Name.Replace("$PlaceHolderCompanyName.$PlaceHolderProjectName",$NewProjectName).Replace($PlaceHolderCompanyName,$NewProjectName).Replace($PlaceHolderProjectName,$NewProjectName)
	...
	Ls $TargetFolder -Include $include -Recurse | Where { $_.GetType().Name -eq $fileType} | ForEach-Object{
		...
		if($fileText.Length -gt 0 -and ($fileText.contains($PlaceHolderCompanyName) -or $fileText.contains($PlaceHolderProjectName))){
			$fileText.Replace("$PlaceHolderCompanyName.$PlaceHolderProjectName",$NewProjectName).Replace($PlaceHolderCompanyName,$NewProjectName).Replace($PlaceHolderProjectName,$NewProjectName) | Set-Content $_ -Encoding UTF8
		...
		If($_.Name.contains($PlaceHolderCompanyName) -or $_.Name.contains($PlaceHolderProjectName)){
			$newFileName=$_.Name.Replace("$PlaceHolderCompanyName.$PlaceHolderProjectName",$NewProjectName).Replace($PlaceHolderCompanyName,$NewProjectName).Replace($PlaceHolderProjectName,$NewProjectName)
